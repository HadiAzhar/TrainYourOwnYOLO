...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="postfix",
        default="_output",
        help='Specify the postfix for images with bounding boxes. Default is "_output"',
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help='turn on the webcam until the exit button = q is pressed',
    )

    FLAGS = parser.parse_args()

    webcam = FLAGS.webcam_mode
    print("webcam =", webcam)

    save_img = not FLAGS.no_save_img
    print("saved_image =", save_img)
    ...
    input_image_paths = []
    input_video_paths = []
    input_webcam_paths = True

    if not webcam:
        input_webcam_paths = False
    ...
    if input_webcam_paths:
        print("starting webcam prediction ... \n press 'Q' to exit")

        start = timer()
        output_path = os.path.join(FLAGS.output,
                                   os.path.basename("webcam").replace(".", FLAGS.postfix + "."))
        detect_video(yolo, 0, output_path=output_path)

        end = timer()

        print("ending webcam ... timer:{}".format(end - start))
